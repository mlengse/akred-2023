...
*   **Pemetaan Klaster**: **Klaster 2 (Kesehatan Ibu dan Anak)** & **Klaster 3 (Kesehatan Usia Dewasa dan Lansia)** &rarr; Sub-klaster *Kesehatan Keluarga*.
*   **Kaitan dengan Siklus Hidup**: Pelayanan Kesehatan Keluarga (Kesga) dibagi secara fungsional ke dalam siklus hidup. Pelayanan ANC terpadu, pemeriksaan tumbuh kembang balita (SDIDTK), serta pelayanan PKPR remaja dikelola oleh Klaster 2 [18, 19]. Sedangkan pelayanan skrining kesehatan calon pengantin (catin), kontrasepsi PUS, dan pelayanan lansia dikelola oleh Klaster 3 [18].
*   **Dokumen Regulasi & Bukti Pendukung ILP**:
    - **Indikator Kinerja Utama Kesga**: SK Indikator Kinerja menetapkan 6 target utama: cakupan ANC terpadu, standar pelayanan minimal balita, penjaringan anak sekolah/PKPR, skrining catin, cakupan KB PUS, dan pelayanan kesehatan lansia [18].
    - **Aplikasi Pencatatan Digital**: Pelaporan berkala capaian indikator Kesga menggunakan sistem elektronik resmi Kemenkes seperti `e-Kohort KIA`, `SIGA (Sistem Informasi Keluarga)`, serta entry skrining bulanan lansia/catin pada aplikasi `ASIK` [24].

### 📋 Teknik Survei & Penyusunan Rekomendasi (Best Practices)

#### Metode Pembuktian RDOWS:
*   **[R] Regulasi**: Periksa SK Target Kinerja UKM Kesga, SOP Pelayanan KIA/KB/Remaja/Lansia, serta KAK Pelayanan Kesga.
*   **[D] Dokumen**: Siapkan data laporan pencapaian indikator kinerja Kesga teranalisis, bukti fisik penjaringan kesehatan (SD Negeri Pajang, SMP/SMA binaan), berkas laporan kemitraan skrining catin bersama KUA Kecamatan Laweyan, printout e-Kohort dan SIGA terupdate, serta berkas rencana tindak lanjut (RTL).
*   **[W] Wawancara**: Wawancara PJ UKM, Koordinator Kesga (Bidan Koordinator), Pelaksana PKPR, Koordinator Lansia, KUA Kecamatan Laweyan, serta kader pendamping.

#### Contoh Fakta Analisis Kontekstual vs Tekstual:
::code-group{data-pagefind-ignore}
```md [❌ Salah (Tekstual)]
Fakta: Indikator kinerja pelayanan UKM Esensial Kesehatan Keluarga telah tercapai sesuai pokok pikiran disertai dengan analisisnya.
Analisis: Laporan pencatatan dan pelaporan elektronik sudah lengkap dan diisi tepat waktu (Skor 5).
```
```md [✓ Benar (Kontekstual)]
Fakta: Telaah dokumen (D) pada aplikasi e-Kohort menunjukkan cakupan ANC Terpadu ibu hamil di Kelurahan Pajang telah mencapai 100%. Namun, berdasarkan rekapitulasi data e-Kohort (D) dan wawancara (W) dengan Koordinator KIA, cakupan Skrining Calon Pengantin (Catin) baru mencapai 82% dari target 90% dikarenakan kendala catin pria yang bekerja di luar daerah. Puskesmas telah menyusun dokumen analisis hambatan (D) dan merencanakan RTL berupa kolaborasi bersama KUA Laweyan untuk skrining virtual (telekonseling) (Skor 5).
Analisis: Puskesmas Pajang tertib secara administrasi dan analisis data Kesga, namun penilaian kinerja fisik belum terpenuhi penuh karena kendala jangkauan sasaran catin pria.
```
::

#### Contoh Rekomendasi Operasional (Kalimat Perintah):
*   **Rekomendasi**: *Tingkatkan cakupan skrining kesehatan Calon Pengantin (Catin) pria di Kelurahan Pajang, Sondakan, dan Laweyan dengan mengoptimalkan kerja sama integratif bersama KUA Kecamatan Laweyan melalui penerapan telekonseling dan verifikasi kesehatan sebelum rekomendasi nikah diterbitkan.*
