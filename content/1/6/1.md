...
*   **Pemetaan Klaster**: **Klaster 1 (Manajemen)** &rarr; Sub-Klaster *Manajemen Kinerja & Lokakarya Mini*.
*   **Kaitan dengan Siklus Hidup**: Monitoring berkala kinerja (PWS) per wilayah kelurahan di bawah Puskesmas Pajang memastikan seluruh indikator siklus hidup (seperti cakupan imunisasi bayi, screening PTM lansia) terpantau dan intervensi dilakukan tepat waktu.
*   **Dokumen Regulasi & Bukti Pendukung ILP**:
    - **SK Indikator Kinerja**: Kebijakan penetapan indikator kinerja Puskesmas Pajang yang diselaraskan dengan target standar pelayanan minimum (SPM) era ILP.
    - **Notulen Lokmin**: Bukti notulen Lokakarya Mini bulanan dan tribulanan lintas sektor yang memuat analisis masalah capaian indikator kesehatan masyarakat.

### 📋 Teknik Survei & Penyusunan Rekomendasi (Best Practices)

#### Metode Pembuktian RDOWS:
*   **[R] Regulasi**: Periksa SK Indikator Kinerja dan target tahunan, SOP Penilaian Kinerja Puskesmas (PKP), dan panduan Audit Internal.
*   **[D] Dokumen**: Siapkan berkas Laporan PKP tahunan, notulen Lokmin lengkap dengan daftar hadir dan foto, serta grafik pemantauan tren capaian bulanan.
*   **[W] Wawancara**: Wawancara KTU dan penanggung jawab mutu mengenai tindak lanjut nyata terhadap program kerja yang capaiannya di bawah target bulanan.

#### Contoh Fakta Analisis Kontekstual vs Tekstual:
::code-group{data-pagefind-ignore}
```md [❌ Salah (Tekstual)]
Fakta: Penilaian kinerja Puskesmas Pajang sudah dilakukan secara rutin (Skor 5).
Analisis: Pembahasan evaluasi kinerja masih perlu ditingkatkan lagi.
```
```md [✓ Benar (Kontekstual)]
Fakta: Penilaian Kinerja Puskesmas (PKP) telah dinilai secara berkala. Namun, tidak ditemukan bukti tindak lanjut tertulis atau intervensi lapangan terhadap indikator cakupan deteksi dini PTM (Klaster 3) yang berada di bawah target SPM 90% pada triwulan pertama (Skor 5).
Analisis: Puskesmas Pajang telah melakukan pengawasan capaian indikator kinerja, namun respon perbaikan terhadap program kesehatan yang tidak mencapai target belum direncanakan secara konkret.
```
::

#### Contoh Rekomendasi Operasional (Kalimat Perintah):
*   **Rekomendasi**: *Susun rencana tindak lanjut (RTL) yang konkret dan laksanakan intervensi lapangan untuk mengejar ketertinggalan capaian indikator deteksi dini PTM Usia Dewasa di Puskesmas Pajang.*
