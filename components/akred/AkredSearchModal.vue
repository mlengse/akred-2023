...
const defaultGroups = computed(() => navigation.value.map((item: { _path: string; title: string }) => ({
  key: item._path,
  label: item.title,
...
const queryGroups = computed(() => navigation.value.map((item: { _path: string; title: string }) => ({
  key: item._path,
  label: item.title,
...
function remapChildren (children: any[]) {
  return children?.map((grandChild) => {
    if (['code-inline', 'em', 'a', 'strong'].includes(grandChild.tag)) {
      return { type: 'text', value: grandChild.children.find((child: { type: string }) => child.type === 'text')?.value || '' }
    }
...
function concatChildren (children: any[]) {
  return children.map((child) => {
    ...
    if (child.tag === 'p') {
      ...
      child.children = child.children?.reduce((acc: any[], grandChild: { type: string; value: any }) => {
...
function groupByHeading (children: any[]) {
  ...
  for (const node of children) {
    ...
    if (['h2', 'h3'].includes(node.tag)) {
      ...
      title = node.children?.find((child: { type: string }) => child.type === 'text')?.value
      if (title) {
        hash = `#${node.props.id}`
      }
    }
